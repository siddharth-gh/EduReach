    ...
    <>
      ...
      >
        <div className="p-8 border-b border-border flex justify-between items-center">
          ...
          <button 
            onClick={onClose}
            className="w-10 h-10 rounded-xl bg-surface flex items-center justify-center text-gray-400"
          >
            ✕
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-6 space-y-2">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              onClick={onClose}
              className={({ isActive }) => `flex items-center gap-4 px-4 py-4 rounded-2xl transition-all font-bold ${isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-secondary hover:bg-gray-50 dark:hover:bg-gray-800'}`}
            >
        ...
        <div className="p-6 border-t border-border space-y-6">
          ...
          <div>
            ...
            <div className="flex bg-surface-soft rounded-2xl p-1">
          ...
          {!isAuthenticated && (
            <div className="grid grid-cols-2 gap-3">
               <NavLink to="/login" onClick={onClose} className="py-4 text-center text-xs font-black uppercase tracking-widest text-gray-500 bg-surface-soft rounded-2xl">Log In</NavLink>
