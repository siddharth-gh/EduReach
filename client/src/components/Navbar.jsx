...
const Navbar = ({ theme, onToggleTheme, onToggleSidebar }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-background/80 backdrop-blur-xl border-b border-border transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <NavLink to="/courses" className={({ isActive }) => `text-sm font-medium transition-colors ${isActive ? 'text-blue-600' : 'text-secondary hover:text-gray-900 dark:hover:text-white'}`}>
              {t("nav.courses")}
            </NavLink>
            <NavLink to="/offline" className={({ isActive }) => `text-sm font-medium transition-colors ${isActive ? 'text-blue-600' : 'text-secondary hover:text-gray-900 dark:hover:text-white'}`}>
              {t("nav.offline")}
            </NavLink>
            ...
            <div className="flex items-center gap-4 ml-4 pl-4 border-l border-border">
              ...
              <div className="flex bg-surface-soft rounded-lg p-1">
                <button onClick={() => handleLanguageChange('en')} className={`px-2 py-1 text-[10px] font-bold rounded-md transition-all ${i18n.language.startsWith('en') ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600' : 'text-gray-500'}`}>Eng</button>
              ...
              {/* Theme Toggle */}
              <button onClick={onToggleTheme} className="w-8 h-8 flex items-center justify-center rounded-lg bg-surface-soft text-sm hover:ring-2 hover:ring-blue-500/20 transition-all">
                {theme === "dark" ? "☀️" : "🌙"}
              </button>
              ...
              {isAuthenticated ? (
                <div className="flex items-center gap-4">
                  <NavLink to={getHomeRouteForRole(user?.role)} className="text-sm font-semibold text-primary">
                    {t("nav.dashboard")}
                  </NavLink>
                ...
                <div className="flex items-center gap-4">
                  <NavLink to="/login" className="text-sm font-medium text-secondary hover:text-gray-900 dark:hover:text-white">
                    {t("nav.login")}
                  </NavLink>
          ...
          <div className="md:hidden flex items-center gap-3">
             <button onClick={onToggleTheme} className="w-10 h-10 flex items-center justify-center rounded-xl bg-surface-soft text-sm">
                {theme === "dark" ? "☀️" : "🌙"}
              </button>
              <button 
