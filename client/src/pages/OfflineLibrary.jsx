    ...
    <AppShell>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        ...
        {offlineCourses.length === 0 ? (
          <div className="text-center py-20 bg-surface/50 rounded-[48px] border-2 border-dashed border-border">
          ...
          <div className="grid grid-cols-1 gap-6">
            {offlineCourses.map((course) => (
              <article key={course.courseId} className="group bg-surface p-8 rounded-[40px] border border-border shadow-xl shadow-blue-600/5 hover:shadow-2xl hover:shadow-blue-600/10 transition-all">
                <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
                  <div>
                    ...
                    <h3 className="text-2xl font-black text-primary group-hover:text-blue-600 transition-colors">
                      {course.title}
                    </h3>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      className="px-6 py-3 bg-surface-soft text-secondary text-xs font-bold rounded-xl hover:bg-red-500 hover:text-white transition-all"
                ...
                <div className="flex items-center gap-6 pt-6 border-t border-gray-50 dark:border-gray-800">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-primary">{course.modules}</span>
                   </div>
                   <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-gray-400 uppercase tracking-widest">{t("offlineLibrary.downloadedAt")}:</span>
                      <span className="text-sm font-bold text-secondary">{new Date(course.downloadedAt).toLocaleDateString()}</span>
                   </div>
                </div>
              </article>
            ))}
