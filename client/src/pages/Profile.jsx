  ...
  return (
    <AppShell>
      <div className="max-w-4xl mx-auto px-4 py-4 lg:py-8">
         <div className="bg-surface rounded-[48px] border border-border p-8 lg:p-16 shadow-2xl shadow-blue-600/5 transition-colors duration-200">
            ...
            <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 pb-10 border-b border-gray-50 dark:border-gray-800">
               <div>
                  ...
                  <h1 className="text-3xl lg:text-4xl font-black text-primary mt-2 leading-tight">{user?.name || user?.email}</h1>
                  <div className="flex gap-4 mt-4">
                     <span className="px-3 py-1 bg-surface text-gray-400 text-[10px] font-black rounded-full uppercase tracking-widest">{t("profile.role")}: {user?.role}</span>
                     {user?.role === "student" && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
               ...
               <div className="lg:col-span-2">
                  <h3 className="text-xl font-bold text-primary mb-8">Account Details</h3>
                  <form onSubmit={handleSubmit} className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                           ...
                           <input
                             className="w-full px-5 py-4 rounded-2xl bg-surface border-none text-primary placeholder-gray-400 focus:ring-2 focus:ring-blue-500 transition-all"
                             value={name}
                        ...
                        <div>
                           ...
                           <select
                             className="w-full px-5 py-4 rounded-2xl bg-surface border-none text-secondary font-bold text-sm focus:ring-2 focus:ring-blue-500 transition-all appearance-none"
                             value={preferredMode}
                     ...
                     <div>
                        ...
                        <textarea
                          className="w-full px-5 py-4 rounded-2xl bg-surface border-none text-primary placeholder-gray-400 focus:ring-2 focus:ring-blue-500 transition-all resize-none"
                          rows={4}
                          value={bio}
               ...
               {user?.role === "student" && (
                 <div className="lg:col-span-1">
                    <h3 className="text-xl font-bold text-primary mb-8">{t("profile.achievements")}</h3>
                    <div className="space-y-4">
                       {achievements.length === 0 ? (
                         <div className="p-8 bg-surface/50 rounded-3xl border-2 border-dashed border-border text-center text-gray-400 italic text-sm">
                            No achievements yet. Start learning!
                         </div>
                       ) : (
                         achievements.map((a) => (
                           <div key={a._id} className="p-5 bg-surface/50 rounded-3xl border border-border flex items-start gap-4 group hover:border-blue-500 transition-all">
                              <div className="w-10 h-10 bg-surface rounded-xl flex items-center justify-center text-xl shadow-sm group-hover:scale-110 transition-transform">🏅</div>
                              <div>
