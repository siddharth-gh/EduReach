...
const QuizResult = () => {
  ...
  if (!attempt) return <AppShell><div className="max-w-3xl mx-auto px-4 py-20 animate-pulse"><div className="h-48 bg-surface-soft/50 rounded-[48px] mb-12"></div><div className="space-y-6"><div className="h-32 bg-surface/20 rounded-3xl"></div><div className="h-32 bg-surface/20 rounded-3xl"></div></div></div></AppShell>;
    ...
    <AppShell>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        ...
        <header className="text-center mb-16">
           ...
           <h1 className="text-4xl font-black text-primary mb-6">{attempt.quizId?.title}</h1>
           
           <div className="inline-flex flex-col items-center p-10 bg-surface border border-border rounded-[48px] shadow-2xl shadow-blue-600/5 relative overflow-hidden">
              <div className={`absolute top-0 inset-x-0 h-2 ${attempt.passed ? 'bg-green-500' : 'bg-red-500'}`} />
              <p className="text-5xl font-black text-primary mb-2">{attempt.score}%</p>
            ...
            return (
              <article key={index} className={`p-8 lg:p-10 rounded-[40px] border transition-all ${isCorrect ? 'bg-green-50/20 dark:bg-green-900/10 border-green-100 dark:border-green-900/20' : 'bg-red-50/20 dark:bg-red-900/10 border-red-100 dark:border-red-900/20'}`}>
                <div className="flex gap-4 mb-6">
                   ...
                   <h3 className="text-lg font-bold text-primary">{index + 1}. {question.questionText}</h3>
                </div>
                ...
                <div className="space-y-4 ml-12">
                   ...
                   {!isCorrect && (
                     <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-primary">{question.options[question.correctAnswer]}</span>
                     </div>
                   )}
                   {question.explanation && (
                     <div className="mt-4 p-4 bg-white/50 bg-surface-soft/50 rounded-2xl border border-black/5 dark:border-white/5">
                        <p className="text-xs italic text-secondary"><span className="font-bold not-italic">{t("quizResult.explanation")}:</span> {question.explanation}</p>
                     </div>
                   )}
                </div>
              </article>
